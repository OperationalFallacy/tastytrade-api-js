...
export default class MarketDataStreamer {
    ...
    addSubscription(symbol, options = {
        ...
        subscriptionTypes = subscriptionTypes.filter((type) => type !== MarketDataSubscriptionType.Candle);
    ...
    removeSubscription(symbol, options = {
        subscriptionTypes: AllSubscriptionTypes,
        channelId: DefaultChannelId,
    }) {
    ...
    dequeueSubscription(symbol, options) {
        ...
        if (queue) {
            const filtered = queue.filter((queueItem) => queueItem.symbol !== symbol);
            queue.length = 0;
            queue.push(...filtered);
        }
    }
    ...
    handleMessageReceived(data) {
        const messageData = "data" in data ? data.data : "{}";
