...
export default class TransactionsService {
  ...
  async getTransaction(accountNumber: string, id: string) {
    ...
    const symbolData = await this.httpClient.getData(
  ...
  async getTotalFees(accountNumber: string) {
    ...
    const totalFees = await this.httpClient.getData(
      `/accounts/${accountNumber}/transactions/total-fees`,
      {},
      {}
    );
    return totalFees;
  ...
  async getAccountTransactions(accountNumber: string, queryParams = {}) {
    ...
    const accountTransactions = await this.httpClient.getData(
