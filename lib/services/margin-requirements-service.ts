...
export default class MarginRequirementsService {
  ...
  async getMarginRequirements(accountNumber: string) {
    ...
    const marginRequirements = await this.httpClient.getData(
      `/margin/accounts/${accountNumber}/requirements`
    );
  ...
  async postMarginRequirements(accountNumber: string, order: object) {
    ...
    const marginRequirements = await this.httpClient.postData(
      `/margin/accounts/${accountNumber}/dry-run`,
      order,
      {}
    );
    return marginRequirements;
