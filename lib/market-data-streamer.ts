...
export default class MarketDataStreamer {
  ...
  constructor() {
    console.warn(
      "MarketDataStreamer is deprecated and will be removed in a future release of @tastytrade/api. Use @dxfeed/dxlink-api instead."
    );
  }

  addDataListener(
    dataListener: MarketDataListener,
    channelId: number | null = null
  ): Remover {
    if (isNil(dataListener)) {
      return () => {};
    }
    const guid = uuidv4();
  ...
  connect(url: string, token: string) {
    if (this.isConnected) {
      throw new Error(
        "MarketDataStreamer is attempting to connect when an existing websocket is already connected"
      );
    }
  ...
  addSubscription(
    ...
    subscriptionTypes = subscriptionTypes.filter(
      (type) => type !== MarketDataSubscriptionType.Candle
    );
  ...
  addCandleSubscription(
    ...
    if (isOpen) {
      ...
      this.queueSubscription(candleSymbol, {
        subscriptionTypes,
        channelId,
        subscriptionArgs,
      });
  ...
  openFeedChannel(channelId: number) {
    if (!this.isReadyToOpenChannels) {
      throw new Error(
        `Unable to open channel ${channelId} due to DxLink authorization state: ${this.authState}`
      );
    }
  ...
  private sendKeepalive() {
    if (isNil(this.keepaliveIntervalId)) {
      return;
    }
  ...
  private dequeueSubscription(symbol: string, options: SubscriptionOptions) {
    ...
    if (queue) {
      const filtered = queue.filter(
        (queueItem: any) => queueItem.symbol !== symbol
      );
      queue.length = 0;
      queue.push(...filtered);
    }
  ...
  private sendSubscriptionMessage(
    ...
    const subscriptions = subscriptionTypes.map((type) =>
      Object.assign({}, { symbol: symbol, type: type }, subscriptionArgs ?? {})
    );
    this.sendMessage({
      type: "FEED_SUBSCRIPTION",
      channel: channelId,
  ...
  private handleAuthStateMessage(data: any) {
    ...
    this.authStateListeners.forEach((listener) =>
      listener(this.isDxLinkAuthorized)
    );
  ...
  private notifyListeners(jsonData: any) {
    this.dataListeners.forEach((listenerData) => {
      if (
        listenerData.channelId === jsonData.channel ||
        isNil(listenerData.channelId)
      ) {
