...
export default class WatchlistsService {
  ...
  async getPairsWatchlists() {
    ...
    const pairsWatchlists = await this.httpClient.getData(
      `/pairs-watchlists`,
      {},
      {}
    );
    return pairsWatchlists;
  }
  async getPairsWatchlist(pairsWatchlistName: string) {
    //Returns a requested tastyworks pairs watchlist
    const pairsWatchlist = await this.httpClient.getData(
      `/pairs-watchlists/${pairsWatchlistName}`,
      {},
      {}
    );
    return pairsWatchlist;
  }
  ...
  async getPublicWatchlists(countsOnly = false) {
    ...
    const publicWatchlists = await this.httpClient.getData(
      `/public-watchlists`,
      {},
      { "counts-only": countsOnly }
    );
    return publicWatchlists;
  }
  async getPublicWatchlist(watchlistName: string) {
    //Returns a requested tastyworks watchlist
    const publicWatchlist = await this.httpClient.getData(
      `/public-watchlists/${watchlistName}`,
      {},
      {}
    );
  ...
  async createAccountWatchlist(watchlist: object) {
    ...
    const accountWatchlist = await this.httpClient.postData(
      `/watchlists`,
      watchlist,
      {}
    );
  ...
  async replaceWatchlist(watchlistName: string, replacementWatchlist: object) {
    ...
    const watchlist = await this.httpClient.putData(
  ...
  async deleteWatchlist(watchlistName: string) {
    ...
    const watchlist = await this.httpClient.deleteData(
      `/watchlists/${watchlistName}`,
      {}
    );
    return watchlist;
  ...
  async getSingleWatchlist(watchlistName: string) {
    ...
    const singleWatchlist = await this.httpClient.getData(
      `/watchlists/${watchlistName}`,
      {},
      {}
    );
