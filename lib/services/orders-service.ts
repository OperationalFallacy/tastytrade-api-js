...
export default class OrderService {
  ...
  async postReconfirmOrder(accountNumber: string, orderId: number) {
    //Reconfirm an order
    const reconfirmOrder = await this.httpClient.postData(
      `/accounts/${accountNumber}/orders/${orderId}/reconfirm`,
      {},
      {}
    );
  ...
  async replacementOrderDryRun(
    ...
    const replacementOrderDryRun = await this.httpClient.postData(
      `/accounts/${accountNumber}/orders/${orderId}/dry-run`,
      replacementOrder,
      {}
    );
  ...
  async getOrder(accountNumber: string, orderId: number) {
    ...
    const order = await this.httpClient.getData(
  ...
  async cancelOrder(
    ...
    const order = await this.httpClient.deleteData(
      `/accounts/${accountNumber}/orders/${orderId}`,
      {}
    );
    return extractResponseData(order);
  ...
  async replaceOrder(
    ...
    const order = await this.httpClient.putData(
      `/accounts/${accountNumber}/orders/${orderId}`,
      replacementOrder,
      {}
    );
  ...
  async editOrder(accountNumber: string, orderId: number, order: object) {
    ...
    const orderResponse = await this.httpClient.patchData(
      `/accounts/${accountNumber}/orders/${orderId}`,
      order,
      {}
    );
  ...
  async getLiveOrders(accountNumber: string) {
    ...
    const liveOrders = await this.httpClient.getData(
  ...
  async getOrders(
    ...
    const orders = await this.httpClient.getData(
  ...
  async createOrder(
    ...
    const orderResponse = await this.httpClient.postData(
      `/accounts/${accountNumber}/orders`,
      order,
      {}
    );
  ...
  async postOrderDryRun(
    ...
    const orderDryRun = await this.httpClient.postData(
  ...
  async getLiveOrdersForCustomer(customerId: string) {
    //Returns a list of live orders for the resource
    const liveOrders = await this.httpClient.getData(
      `/customers/${customerId}/orders/live`,
      {},
      {}
    );
    return extractResponseData(liveOrders);
  ...
  async getCustomerOrders(customerId: string, queryParams = {}) {
    //Returns a paginated list of the customer's orders (as identified by the provided authentication token) based on sort param. If no sort is passed in, it defaults to descending order.
    const customerOrders = await this.httpClient.getData(
